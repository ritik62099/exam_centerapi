...
const getExamResults = async (req, res) => {
  try {
    const results = await Result.find({ exam: req.params.examId })
      .populate(
  "student",
  "name rollNumber registrationNumber mobileNumber username photo"
)
      .populate({
        path: "exam",
        populate: [
...
const getStudentResultsByAdmin = async (req, res) => {
  try {
    const results = await Result.find({ student: req.params.studentId })
      .populate(
        "student",
        "name username rollNumber registrationNumber mobileNumber photo"
      )
    ...
    console.log("Get student results by admin error:", error);

    res.status(500).json({
      success: false,
      message: error.message || "Student results fetch nahi ho paya",
    });
  }
};
